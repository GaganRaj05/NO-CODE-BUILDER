...
class SpecGenerator:
    def __init__(self):
        ...
        self.frontend_parser = PydanticOutputParser(pydantic_object=FrontendSpec)
        self.backend_parser = PydanticOutputParser(pydantic_object=BackendSpec)
        ...
        self.frontend_retry_parser = RetryWithErrorOutputParser.from_llm(
            parser=self.frontend_parser, llm=self.llm, max_retries=2
        )
        ...
        self.backend_retry_parser = RetryWithErrorOutputParser.from_llm(
            llm=self.llm, max_retries=2, parser=self.backend_parser
        )

        self.frontend_chain = LLMChain(llm=self.llm, prompt=self.frontend_spec_prompt)
        self.backend_chain = LLMChain(llm=self.llm, prompt=self.backend_spec_prompt)

    async def generate_complete_spec(self, context: RequirementContext) -> CompleteSpec:
        try:
            requirements = {
                q_id: ans.answer for q_id, ans in context.answered_questions.items()
            }
            ...
            frontend_spec = self.frontend_retry_parser.parse_with_prompt(
                raw_frontend_spec,
                self.frontend_spec_prompt.format(requirements=requirements),
            )
            raw_backend_spec = await self.backend_chain.arun(requirements=requirements)
            backend_spec = await self.backend_retry_parser.parse_with_prompt(
                raw_backend_spec, self.backend_spec_prompt.format(requirements=requirements)
            )
            complete_spec = CompleteSpec(
                ...
                integration_points= await self._generate_integration_points(
    ...
    async def _generate_integration_points(
        self, frontend_spec: FrontendSpec, backend_spec: BackendSpec
    ) -> List[str]:
        try:
            integration_points = []
            ...
            logger.error(
                f"Integration points generation failed with an error:\n{str(e)}"
            )
            raise e
